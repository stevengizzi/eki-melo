/* =================================================================
   ...
   Flow:
     1. Stage 6  — realizeVoices → VoiceTracks (Pitch objects throughout).
     2. Stage 7  — applyVoiceLeading (identity stub until Session 7).
     3. Stage 8  — enforceCadences  (identity stub until Session 5).
     4. Render   — collapse each event's Pitch to a synth string via
                   toSynthString, choosing sharp/flat from the tonic. This is
                   the ONLY place Pitch → string conversion happens.
     5. Assemble — the FinalJingle metadata (title, tempo, key, mood, form,
...
import { realizeVoices, computeSectionPlan } from './stage-6-voice.js';
...
// Collapse a VoiceTracks voice (Pitch objects / rests) into the synth's
// [pitchString, duration] tuples. Rests (pitch === null) become "rest".
...
export function runPipeline(input, config = DEFAULT_CONFIG) {
  ...
  voiceTracks = enforceCadences(voiceTracks, input.harmonicPlan, macroParams);
  ...
  return {
    ...
    lead: renderTrack(voiceTracks.lead, preference),
    harmony: renderTrack(voiceTracks.harmony, preference),
    bass: renderTrack(voiceTracks.bass, preference),
  };
}
